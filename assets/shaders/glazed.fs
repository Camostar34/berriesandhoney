...
extern MY_HIGHP_OR_MEDIUMP vec2 redglaze;
extern MY_HIGHP_OR_MEDIUMP vec2 blueglaze;
...
extern Image aux_img;
extern float aux_num;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float cx = uv_scaled_centered.x * 0.3;
    float cy = uv_scaled_centered.y * 0.3;


    vec2 aux_coords = vec2(
        mod2( aux_num*171. + texture_coords.x/(atlasScaleXY.x * 71), 1 ), 
        mod2( aux_num*229. + texture_coords.y/(atlasScaleXY.y * 95), 1 ));


    vec4 aux_tex = Texel(aux_img, aux_coords);

    float stripe1 = (sign(2.4 - abs(cy + cx/4 - 8.5 + glazed.x * 3.5)) + 1)/2;
    float stripe2 = (sign(0.9 - abs(cy + cx/4 - 3.5 + glazed.x * 3.9)) + 1)/2;

    tex.rgb += stripe1/14 + stripe2/14;

    float ovl = (pow(tex.r, 2) + pow(tex.g, 2) + pow(tex.b, 2) + 0.3)/4;

    tex.rgb = tex.rgb * (1 - ovl) + aux_tex.rgb * ovl;

    tex.rgb += stripe1/7 + stripe2/8;
