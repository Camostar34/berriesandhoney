...
SMODS.Consumable {
    ...
    loc_vars = function(self, info_queue, card)
        info_queue[#info_queue + 1] = G.P_CENTERS[card.ability.mod_conv]
        return {
            vars = { card.ability.extra.luck, card.ability.extra.berry_mult, card.ability.extra.activity },
         }
    end,
...
SMODS.Consumable {
    ...
    can_use = function(self, card) return G.STATES.SELECTING_HAND end,

    use = function(self, card, area)
        local hand_space = math.min(#G.deck.cards, card.ability.extra.draw_mod)
        for i = 1, hand_space do draw_card(G.deck, G.hand, i * 100 / hand_space, "up", true) end
    end,
}
